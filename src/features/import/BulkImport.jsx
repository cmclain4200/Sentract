...
export default function BulkImport({ onClose, onImported, userId }) {
      ...
      if (ext === "csv") {
        ...
        try {
          ...
          if (parsed.type === "sentract") {
            ...
            setRawData({
              headers: Object.keys(parsed.records[0] || {}),
              rows: parsed.records.map((r) => Object.values(r)),
              raw: parsed.records,
            });
            const autoMap = {};
  ...
  async function handleImport() {
    ...
    try {
      ...
      if (targetCaseId === "new") {
        ...
        if (caseErr || !newCase) {
          setImportResult({ error: caseErr?.message || "Failed to create case" });
          setStep(3);
          setImporting(false);
          return;
        }
        ...
        // Create case assignment for the creator
        if (userId) {
      ...
      if (created === 0 && profiles.length > 0) {
        setImportResult({ error: lastError || "All subject imports failed" });
      } else {
  ...
  return (
    ...
    >
      <div
        className="surface w-full max-w-2xl max-h-[80vh] flex flex-col fade-in"
          ...
          {step === 1 && rawData && !rawData.error && (
            <div>
              ...
              <button
                ...
                className="mt-6 w-full py-2.5 rounded text-sm font-semibold cursor-pointer"
          ...
          {step === 2 && (
            <div>
              ...
              <div className="space-y-2 mb-6 max-h-[300px] overflow-y-auto">
                {profiles.slice(0, 20).map((p, i) => (
                  ...
                  >
                    <div className="text-[13px] text-white">
                      {p.identity?.full_name || p.name || `Subject ${i + 1}`}
                    </div>
                    <div className="text-[11px]" style={{ color: "#555" }}>
                      {[
                        p.professional?.title,
                        p.professional?.organization,
                        p.contact?.email_addresses?.[0]?.address,
                      ]
                        .filter(Boolean)
                        .join(" \u00b7 ")}
                    </div>
                ...
                {profiles.length > 20 && (
                  <div className="text-[11px] text-center" style={{ color: "#555" }}>
                    ...and {profiles.length - 20} more
                  </div>
                )}
              </div>
              ...
              <button
                ...
                style={{
                  background: importing ? "#1a1a1a" : "#09BC8A",
                  color: importing ? "#555" : "#0a0a0a",
                  border: "none",
                }}
              >
                {importing
                  ? "Importing..."
                  : `Import ${profiles.length} Subject${profiles.length !== 1 ? "s" : ""}`}
              </button>
            </div>
          )}
            ...
            <div className="text-center py-8">
              {importResult.error ? (
                ...
                <>
                  ...
                  <button
                    onClick={() => {
                      onImported?.(importResult.caseId);
                    }}
                    className="mt-6 px-6 py-2.5 rounded text-sm font-semibold cursor-pointer"
                    style={{ background: "#09BC8A", color: "#0a0a0a", border: "none" }}
                  >
                    View Case
                  </button>
