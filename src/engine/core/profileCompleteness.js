...
const SECTIONS = [
  ...
  },
  {
    ...
    score: (d) => {
      ...
      const observations = d.behavioral?.observations || [];
      const totalItems = routines.length + observations.length;
      if (totalItems === 0) return 0;
      const routineScore = routines.length > 0
        ? clamp(routines.length / 4, 1) * 0.4 +
          routines.reduce((sum, r) => sum + countFilledFields(r, ['name', 'schedule', 'location', 'consistency', 'data_source']) / 5, 0) / Math.max(routines.length, 1) * 0.2
        : 0;
      const obsScore = observations.length > 0
        ? clamp(observations.length / 4, 1) * 0.25 +
          observations.reduce((sum, o) => sum + countFilledFields(o, ['description', 'exploitability', 'category', 'data_source']) / 4, 0) / Math.max(observations.length, 1) * 0.15
        : 0;
      return routineScore + obsScore;
    },
  },
  {
    key: 'enriched',
